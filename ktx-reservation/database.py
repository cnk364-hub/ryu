...
DB_PATH = os.environ.get("DATABASE_PATH", "ktx.db")
KEY_PATH = os.environ.get("FERNET_KEY_PATH", os.path.join(os.path.dirname(DB_PATH) or ".", ".fernet.key"))

_lock = threading.RLock()


def _load_or_create_key() -> bytes:
    """Fernet 암호화 키를 로드하거나 생성"""
    # 환경변수 우선 (Fly.io secrets 사용 가능)
        ...
        if row is None:
            conn.execute(
                """INSERT INTO settings (id, dep_station, arr_station, dep_date,
                   ...
                   VALUES (1, '서울', '부산', '', '06:00', '22:00', 'ANY', 1, ?)""",
...
def list_profiles():
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT id, name, korail_id, is_selected, created_at FROM profiles ORDER BY id"
        ).fetchall()
        return [dict(r) for r in rows]


def create_profile(name: str, korail_id: str, password: str) -> int:
    with get_conn() as conn:
        ...
        cur = conn.execute(
            """INSERT INTO profiles (name, korail_id, password_enc, is_selected, created_at)
               VALUES (?, ?, ?, 0, ?)""",
            (name, korail_id, enc, datetime.utcnow().isoformat()),
        )
...
def update_profile(profile_id: int, name: str, korail_id: str, password: Optional[str]):
    with get_conn() as conn:
        if password:
            conn.execute(
                "UPDATE profiles SET name=?, korail_id=?, password_enc=? WHERE id=?",
                (name, korail_id, encrypt_password(password), profile_id),
            )
        else:
            conn.execute(
                "UPDATE profiles SET name=?, korail_id=? WHERE id=?",
                (name, korail_id, profile_id),
            )
...
def delete_profile(profile_id: int):
    with get_conn() as conn:
        ...
        # 선택된 프로필이 지워졌을 경우 남은 것 중 첫 번째 선택
...
def get_selected_profile_with_password():
    """엔진에서 사용하는 현재 선택된 프로필 (평문 비밀번호 포함)"""
    with get_conn() as conn:
        row = conn.execute(
            "SELECT id, name, korail_id, password_enc FROM profiles WHERE is_selected=1 LIMIT 1"
        ).fetchone()
        if not row:
            return None
        return {
