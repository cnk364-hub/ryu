...
(function () {
    ...
    async function loadSettings() {
        try {
            ...
            fillStations($('#dep_station'), s.dep_station || '수서');
            ...
            profiles.forEach(p => {
                ...
                el.innerHTML = `
                    <div class="info">
                        ...
                        <div class="korail-id">${escapeHtml(p.srt_id)}</div>
                    </div>
                    <div class="actions">
                        ${p.is_selected ? '' : `<button class="btn btn-sm btn-outline-info" data-act="select" data-id="${p.id}"><i class="bi bi-check2"></i></button>`}
                        <button class="btn btn-sm btn-outline-secondary" data-act="edit" data-id="${p.id}" data-name="${escapeHtml(p.name)}" data-sid="${escapeHtml(p.srt_id)}"><i class="bi bi-pencil"></i></button>
    ...
    $('#profile-form').addEventListener('submit', async (ev) => {
        ...
        const sid = $('#srt_id').value.trim();
        const pw = $('#srt_pw').value;
        try {
            if (id) {
                await api('PUT', `/api/profiles/${id}`, { name, srt_id: sid, password: pw || null });
                ...
                await api('POST', '/api/profiles', { name, srt_id: sid, password: pw });
    ...
    $('#btn-login-test').addEventListener('click', async () => {
        ...
        try {
            const r = await api('POST', '/api/login-test', { srt_id: sid, password: pw });
    ...
    async function loadSummary() {
        ...
        try {
            ...
            $('#summary-profile').textContent = sel ? `${sel.name} (${sel.srt_id})` : '(없음)';
        } catch (e) { /* ignore */ }
    }
    ...
    async function init() {
        try {
            ...
            SRT_STATIONS.length = 0;
            stations.forEach(s => SRT_STATIONS.push(s));
        } catch (e) {}
